...
this hexagonal region, which has area $\Omega$:
%
\begin{equation}
\begin{split}
\frac{1}{\Omega} \oint_c \mb{f}\vcdot d\hat{\mathbf{n}} & \approx \frac{1}{\Omega} \sum_{j=1}^{6} \frac{\mb{f}_j + \mb{f}_0}{2}\vcdot d\hat{\mathbf{n}} \\
& = \frac{1}{\Omega} \sum_{j=1}^{6} \frac{\mb{f}_j^x + \mb{f}_0^x}{2} \vcdot d\hat{\mathbf{n}} +  \sum_{j=1}^{6} \frac{\mb{f}_j^y + \mb{f}_0^y}{2} \vcdot d\hat{\mathbf{n}} \\
& = \frac{1}{\Omega} \sum_{j=1}^{6} \frac{\mb{f}_j^x + \mb{f}_0^x}{2} \cos (60\times(j-1)) + \frac{1}{\Omega} \sum_{j=1}^{6} \frac{\mb{f}_j^y + \mb{f}_0^y}{2} \sin (60\times(j-1)) \\
\end{split}
\end{equation}
...
$a_i(\mb{p}_0,t)\nabla\vcdot\mb{g}(\mb{p}_0)$ can be written (using the fact that the area of a regular hexagon in a lattice with spacing $d$ is $\frac{\sqrt{3}}{2}d^2$):
%
\begin{equation}
\begin{split}
\frac{1}{\Omega} \iint_A a_i\nabla\vcdot\mb{g}\;dxdy & = \frac{a_i(\mb{p}_0)}{\Omega}  \oint_c \mb{g}\vcdot d\hat{\mathbf{n}} \\
& \approx \frac{a_i(\mb{p}_0,t)}{\Omega} \sum_{j=1}^{6} \frac{\mb{g}_j + \mb{g}_0}{2}\vcdot d\hat{\mathbf{n}} \\
& = \frac{2 a_i(\mb{p}_0,t)}{\sqrt{3}d^2} \bigg( \sum_{j=1}^{6} \frac{\mb{g}_j^x + \mb{g}_0^x}{2} \vcdot d\hat{\mathbf{n}} + \frac{1}{\Omega}  \sum_{j=1}^{6} \frac{\mb{g}_j^y + \mb{g}_0^y}{2} \vcdot d\hat{\mathbf{n}} \bigg) \\
& = \frac{a_i(\mb{p}_0,t)}{\sqrt{3}d^2} \bigg( \sum_{j=1}^{6} \big(\mb{g}_j^x + \mb{g}_0^x\big) \cos (60\times(j-1)) + \sum_{j=1}^{6} \big({\mb{g}_j^y + \mb{g}_0^y}\big) \sin (60\times(j-1)) \bigg) \\
\end{split}
\end{equation}
%
...
I think this means that I can ensure that $\mb{J}$ resulting from the
first two terms remains 0, and then I can tailor $\mb{g}(\mb{x})$ so
that it goes to 0 at the boundary, ensuring that the third term
doesn't contribute to $\mb{J}$ at the boundary.
...
is true? Alternatively, can I brute-force the boundary hexes to have
$\nabla\vcdot\mb{J}_i(\mb{x}) = 0$ or $\mb{J}_i(\mb{x}) = 0$ at all
times?

My current hope is that approach 2 for computing the divergence of
$\mb{J}$ will allow me to ensure $\mb{J}$ remains 0 on the boundary.
